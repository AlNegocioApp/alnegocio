...
import { TRIAL_DAYS, HAS_BACKEND } from "./config";
...
export function AuthScreens() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-emerald-950 text-slate-100">
      {/* Brillos decorativos flotantes */}
      <div className="float-glow pointer-events-none absolute -left-32 -top-32 h-96 w-96 rounded-full bg-emerald-500/20 blur-3xl" />
      <div className="float-glow pointer-events-none absolute -bottom-40 right-0 h-[28rem] w-[28rem] rounded-full bg-sky-500/10 blur-3xl" style={{ animationDelay: "2s" }} />
...
Con <BrandName accent="text-emerald-400" /><br />administra tu negocio fácil.
            ...
            <ul className="mt-6 space-y-2 text-sm text-emerald-50/90">
              ...
              <li className="flex items-center gap-2"><Dot /> {TRIAL_DAYS} días de prueba gratis, sin compromiso</li>
          ...
          <div className="anim-scale-in rounded-[2rem] bg-white/95 p-6 text-slate-900 shadow-[0_30px_80px_-20px_rgba(0,0,0,0.5)] backdrop-blur-xl">
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-3">
        <p className="text-sm font-bold text-slate-900">Empiezas con el plan Gratis</p>
        <p className="text-xs text-slate-500">{TRIAL_DAYS} días con todas las funciones. Luego eliges tu plan.</p>
      </div>
      {error && <p className="rounded-lg bg-red-50 px-3 py-2 text-sm font-medium text-red-600">{error}</p>}
