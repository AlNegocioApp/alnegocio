...
import { Card, Button, Select, NumberInput, Modal, Badge, EmptyState, Stat } from "../components/ui";
...
export function Ventas() {
  ...
  return (
    <div className="space-y-5">
      ...
      {pos && <POSModal onClose={() => setPos(false)} onSubmit={(d) => { recordSale(d); setPos(false); setSuccess(true); }} />}
      {success && <SuccessToast message="¡Venta registrada!" onDone={() => setSuccess(false)} />}
      ...
      <div className="grid gap-5 lg:grid-cols-2">
          ...
          <div className="max-h-[320px] space-y-1.5 overflow-y-auto pr-1">
            {filtered.map((p) => (
              <button
                ...
                onClick={() => add(p.id)}
                disabled={p.stock <= 0}
                className="press flex w-full items-center justify-between rounded-2xl border border-black/10 px-3.5 py-3 text-left transition-apple hover:border-emerald-400 hover:bg-emerald-50 disabled:opacity-40"
              >
                <div>
        ...
        <div className="flex flex-col">
          ...
          <div className="mt-3 grid grid-cols-2 gap-3">
            <NumberInput label="Descuento (CUP)" min={0} value={discount} onChange={setDiscount} />
          ...
          <div className="mt-3">
            <NumberInput label="¿Con cuánto paga el cliente? (opcional)" min={0} value={paidWith} onChange={setPaidWith} placeholder="Ej: 1000" />
            {paidWith > 0 && (
              <div className={cn("mt-2 flex items-center justify-between rounded-2xl px-4 py-3 text-sm font-bold", paidWith >= total ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700")}>
                <span>{paidWith >= total ? "Vuelto a entregar" : "Falta por cobrar"}</span>
                <span className="text-lg">{cup(Math.abs(paidWith - total))}</span>
              </div>
            )}
          </div>

          <div className="mt-3 space-y-1 rounded-2xl bg-slate-900 p-4 text-white">
