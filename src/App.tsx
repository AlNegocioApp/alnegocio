...
const nav: Array<{ key: ModuleKey; label: string; Icon: (p: { className?: string }) => ReactNode }> = [
  ...
  { key: "elaborados", label: "Elaborados", Icon: ChefIcon },
...
const titles: Record<ModuleKey, string> = {
  ...
  elaborados: "Producción de elaborados",
...
function Shell() {
  ...
  return (
    <div className="flex min-h-screen bg-[#f5f5f7] text-slate-900 dark:bg-slate-950 dark:text-slate-100">
          ...
          {visibleNav.map(({ key, label, Icon }) => (
            ...
            >
              ...
              {key === "suscripcion" && status !== "activa" && (
                <span className="ml-auto h-2 w-2 rounded-full bg-red-500 pulse-ring" />
              )}
            </button>
          ))}
        </nav>
      ...
      {menuOpen && <div className="anim-fade-in fixed inset-0 z-30 bg-slate-900/40 backdrop-blur-sm lg:hidden" onClick={() => setMenuOpen(false)} />}
