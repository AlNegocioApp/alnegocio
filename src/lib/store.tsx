...
type StoreValue = {
  ...
  produce: (productId: string, qty: number) => void;
  // contacts
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  // ---- Producción (elaborados): fabrica X unidades → descuenta insumos, suma stock ----
  const produce = useCallback((productId: string, qty: number) => {
    setData((d) => {
      const prod = d.products.find((p) => p.id === productId);
      if (!prod || !prod.recipe || qty <= 0) return d;
      // descontar insumos según la receta × cantidad
      const supplies = d.supplies.map((s) => {
        const ing = prod.recipe!.find((r) => r.supplyId === s.id);
        return ing ? { ...s, stock: Math.max(0, s.stock - ing.qty * qty) } : s;
      });
      // sumar al stock del producto elaborado
      const products = d.products.map((p) => (p.id === productId ? { ...p, stock: p.stock + qty } : p));
      return { ...d, supplies, products };
    });
  }, []);
