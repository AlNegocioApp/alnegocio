...
export type Notif = {
  ...
  actionLabel?: string; // texto del botón (ej. "Renovar por WhatsApp")
  actionHref?: string; // enlace del botón
};
  ...
  if (typeof d === "number") {
    if (d < 0) {
      list.push({
        ...
        level: "danger",
        actionLabel: "Renovar por WhatsApp",
        actionHref: whatsappLink("Hola, quiero renovar mi suscripción de AlNegocio."),
      });
    } else if (d <= 5) {
      list.push({
        id: "sub-porvencer",
